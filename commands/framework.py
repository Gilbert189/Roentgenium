...
# from main.py
def log_entry(entry: str, timestamp=None):
...
class CommandInput:
    ...
    def get_long(self) -> str:
        # FIXME: Need to use f-strings, this way is ugly
        return (
            "(" + self.type + ") " + self.name + (" " + self.desc if self.hasdesc else "")
            + (" (Defaults to " + self.default + ")" if self.hasdefault else "")
        )
...
class Command:
    ...
    getShortHelp = get_short_help

    def get_long_help(self, prefix: str) -> str:
        hold = "[quote] " + prefix + self.name + "\n"
        if len(self.inputs) != 0:
            for i in self.inputs:
                hold += "    " + i.getLong() + "\n"
